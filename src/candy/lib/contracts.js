...
import {
	SUBTRACT_GAS_LIMIT,
	contractAddresses,
	supportedBinancePools,
	supportedEthereumPools,
} from './constants.js'

import UNIV2PairAbi from './abi/uni_v2_lp.json'
import CandyAbi from './abi/candy.json'
...
export class Contracts {
	constructor(provider, networkId, web3, options) {
		...
		this.candy = new this.web3.eth.Contract(CandyAbi)
		this.masterChef = new this.web3.eth.Contract(MasterChefAbi)
		this.weth = new this.web3.eth.Contract(WETHAbi)
		this.wethPrice = new this.web3.eth.Contract(ChainOracle)
		this.candyPrice = new this.web3.eth.Contract(UniOracleABI)

		var _pools =  supportedBinancePools ;//(networkId == (97 || 56) ? supportedBinancePools : supportedEthereumPools)
		this.pools = _pools.map((pool) =>
			Object.assign(pool, {
	...
	setProvider(provider, networkId) {
		...
		setProvider(this.candy, contractAddresses.candy[networkId])
		setProvider(this.masterChef, contractAddresses.masterChef[networkId])
		setProvider(this.weth, contractAddresses.weth[networkId])
		setProvider(this.wethPrice, contractAddresses.wethPrice[networkId])
		setProvider(this.candyPrice, contractAddresses.candyPrice[networkId])
	...
	setDefaultAccount(account) {
		this.candy.options.from = account
		this.masterChef.options.from = account
		this.wethPrice.options.from = account
		this.candyPrice.options.from = account
	}
